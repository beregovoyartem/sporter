...
def render_login_page():
    ...
    *{box-sizing:border-box}
    ...
    .login-card{
        ...
        align-items:center;
        gap:20px;
        box-shadow:0 8px 60px rgba(0,20,80,0.5);
        position:relative;
        overflow:hidden;
    ...
    .login-card-title{
        ...
        color:#dde6f5;letter-spacing:.2px;
        margin:0;
    }
    .login-card-sub{
        font-size:.85em;color:#4a6080;
    ...
    .login-divider::before,.login-divider::after{
        content:'';flex:1;height:1px;background:rgba(79,163,255,0.1);
    }

    /* Кнопка Google — стиль офіційний */
    .stButton > button{
        width:100%!important;
    ...
    # Центрована картка через колонки Streamlit
    ...
    with card_col:
        ...
        # Google SVG логотип перед кнопкою
        st.markdown("""
...
NON_FOOTBALL = [
    # ── Явні види спорту ──────────────────────────────────────────────────────
    # ── Явні види спорту (alt-префікс або підрядок в назві ліги) ─────────────
...
def load_livetv():
    ...
    for a_tag in meta_scope.find_all("a", href=re.compile(r"/eventinfo/\d+")):
        ...
        # Точні префікси alt-тегів з livetv (перше слово до крапки).
        # Список складено на основі реального HTML сайту.
        NON_SPORT_PREFIXES = (
            # ── Зустрічаються у реальному HTML ───────────────────────────────
            "хоккей",           # Хоккей
            "баскетбол",        # Баскетбол
            "теннис",           # Теннис
            "волейбол",         # Волейбол
            ...
            "футзал",           # Футзал
            "гандбол",          # Гандбол
            "регби-лига",       # Регби-лига
            "регби-союз",       # Регби-союз
            "регби",            # Регби (загальний)
            "зимний спорт",     # Зимний спорт (біатлон, лижі тощо)
            "бейсбол",          # Бейсбол
            "керлинг",          # Керлинг
            ...
            "австралийский футбол", # Австралийский футбол
            "конный спорт",     # Конный спорт
            "единоборства",     # Единоборства
            "триатлон",         # Триатлон
            "нетбол",           # Нетбол
            "мма",              # ММА
            # ── Додаткові (не в HTML але можливі) ────────────────────────────
            "futsal",
            "мини-футбол", "мінi-футбол", "міні-футбол", "минифутбол",
